...
public class BorrowDAO {
    ...
    public int insertBorrow(ReaderForm readerForm,BookForm bookForm,String operator){
        //获取系统日期
    	   Date dateU=new Date();
    	   java.sql.Date date1=new java.sql.Date(dateU.getTime());
           long date=(long) dateU.getTime();
          ...
          String sql1="select t.days from tb_bookinfo b left join tb_booktype t on b.typeid=t.id where b.id="+bookForm.getId()+"";
           ...
           //计算归还时间
            ...
            date=date+ldays;
            java.sql.Date backdate=new java.sql.Date(date);


          String sql ="Insert into tb_borrow (readerid,bookid,borrowTime,backTime,operator) values("+readerForm.getId()+","+bookForm.getId()+",'"+date1+"','"+backdate+"','"+operator+"')";
          ...
          try {
            if (rs1.next()) {
               ...
               // java.sql.Date date=new java.sql.Date(dateU.getTime());
                ...
                String sql = "UPDATE tb_borrow SET backtime='" + backdate +
                             "' where id=" + id + "";
